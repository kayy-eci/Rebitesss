...
import { foodHeroScoreByPeriod, periodOptions } from './data';
...
export function FoodHeroScoreGaugeCard() {
  const [period, setPeriod] = useState('30-hari');
  const reduced = useReducedMotion();
  const data = foodHeroScoreByPeriod[period];
  ...
  return (
    <Card>
      <div className="flex items-start justify-between gap-2">
        <h2 className="text-sm font-bold text-charcoal-900">Skor Food Hero</h2>
        <FilterDropdown
          value={period}
          onChange={setPeriod}
      ...
      <div className="relative mx-auto mt-2 h-[200px] w-[200px]">
        <svg
          ...
          role="img"
          aria-label={`Skor Food Hero ${data.score} persen`}
        >
          <circle
            cx={SIZE / 2}
            cy={SIZE / 2}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <p className="mt-1 text-[11px] font-medium text-sage-500">Skor Food Hero</p>
        </div>
      </div>

      <p className="mt-3 text-center text-sm font-medium text-charcoal-900">
        Kamu lebih aktif dari{' '}
        <span className="font-bold text-green-700">{data.score}%</span> pengguna ReBites bulan ini
      </p>
      <p className="mt-1 text-center text-[11px] leading-relaxed text-sage-500">
        Berdasarkan frekuensi & konsistensi pembelian 30 hari terakhir
      </p>
