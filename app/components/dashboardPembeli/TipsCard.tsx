import { ArrowRight, Bell, Moon, ShoppingBasket } from 'lucide-react';
...
const tips = [
  {
    icon: Moon,
    title: 'Waktu terbaik berburu',
    body: 'Porsi surplus paling banyak muncul pukul 19.00–21.00 WIB, saat dapur UMKM mulai menutup.',
  },
  {
    icon: Bell,
    title: 'Gabung waitlist',
    body: 'Aktifkan notifikasi waitlist agar porsi favoritmu tidak kehabisan begitu muncul.',
  },
];
    ...
    <Card className="relative overflow-hidden">
      ...
      <div className="relative flex items-center gap-3">
        <div className="relative flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-sage-100/70">
          <ShoppingBasket className="h-6 w-6 text-green-700" />
          <LeafSprig className="-right-3 -top-3 h-14 w-14 text-green-700/15" />
        </div>
        <div>
          <h2 className="text-sm font-bold text-charcoal-900">Tips Belanja Hemat & Berdampak</h2>
          <p className="mt-0.5 text-xs text-sage-500">Jadikan tiap porsi berarti.</p>
        </div>
      </div>
