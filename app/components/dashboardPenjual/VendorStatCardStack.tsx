...
import { vendorStats } from './data';
import { formatRupiah } from '@/lib/data';
import type { VendorStat } from './types';

function StatCard({ stat }: { stat: VendorStat }) {
  const { ref, value } = useCountUp(stat.value);
  const isMoney = stat.label.includes('Pendapatan');
  const isPorsi = stat.label.includes('Porsi');
...
export function VendorStatCardStack() {
  return (
    <div className="flex h-full flex-col gap-4" aria-label="Ringkasan statistik penjualan">
