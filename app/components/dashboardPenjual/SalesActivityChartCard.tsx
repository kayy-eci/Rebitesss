...
export function SalesActivityChartCard() {
  ...
  const { ref, value } = useCountUp(148);
  ...
  const totalTersisa = salesActivityWeek.reduce(
    (sum, point) => sum + point.tersisa,
    0
  );
  ...
  return (
    <Card className="h-full">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <p className="text-xs text-sage-500">{totalTersisa} porsi tersisa minggu ini</p>
      </div>
      ...
      <p className="sr-only">
        Grafik aktivitas penjualan mingguan: 148 porsi terjual, {totalTersisa} porsi tersisa.
      </p>
    </Card>
  );
}
