...
import { incomingOrders, orderPeriodOptions } from './data';
...
export function IncomingOrderListCard() {
  const [period, setPeriod] = useState('7-hari');

  const filtered = period === '7-hari' ? incomingOrders.slice(0, 7) : incomingOrders;

  return (
    <Card className="flex flex-col">
      <div className="flex items-center justify-between gap-2">
        <h2 className="text-sm font-bold text-charcoal-900">Pesanan Masuk</h2>
        <FilterDropdown
          value={period}
          onChange={setPeriod}
          options={orderPeriodOptions}
          ariaLabel="Filter pesanan berdasarkan waktu"
        />
      </div>

      <ul
        className="mt-3 max-h-[600px] divide-y divide-sage-100/60 overflow-y-auto pr-1"
        aria-label="Daftar pesanan masuk"
      >
        {filtered.map((order) => (
          <li key={order.id} className="flex items-center gap-3 py-3">
            <div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-full ring-1 ring-sage-100">
              <SmartImage src={order.customerAvatar} alt={`Foto ${order.customerName}`} />
            </div>
